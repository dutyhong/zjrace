...
class bert_bilstm_crf_model(object):
	...
	def _loader_dict_(self):
		...
		tag_dict = pickle.load(open(os.path.join(self.dict_dir, "ner_tag_dict.pl"), 'rb'))
		self._pos2id = pickle.load(open(os.path.join(self.dict_dir, "ner_pos_dict.pl"), 'rb'))
	...
	def _model_compile_(self):
		...
		in_pos = Input(shape=(self.max_seq_len,), dtype="int32", name="Input_Pos")
		embeds = np.random.normal(0,1,(len(self._tag2id),self.embeds_dim))
		pos_embeds = Embedding(len(self._tag2id),
									self.embeds_dim,
									embeddings_initializer=Constant(embeds),
									input_length=self.max_seq_len,
									trainable=True,
									name="pos_embeds")(in_pos)
		pos_encoder = Bidirectional(LSTM(self.embeds_dim, 
										return_sequences=True, 
										kernel_regularizer=l2_reg))(pos_embeds)
		
		#out_encoder = Concatenate(axis = -1)([trans_sens, pos_encoder])
		w1 = Lambda(lambda x:x*1)(trans_sens)
		w2 = Lambda(lambda x:x*0)(pos_encoder)
		out_encoder = Add()([w1, w2])
		
		bi_lstm_drop = Dropout(0.5)(out_encoder)
		dense = TimeDistributed(Dense(len(self._tag)))(bi_lstm_drop)
		
		crf_tags = CRF(len(self._tag), sparse_target=True)(dense)
		model = Model(inputs=bert_model.inputs+ [in_pos], outputs=crf_tags)
		model.summary()
		model.compile(
	...
	def _model_train_(self, train_text):
		X, y = train_text
		input_ids, input_mask, input_type, input_pos = self._text_process(X)
		...
		self.model.fit(x=[input_ids,input_mask,input_pos], 
					y=trainy, 
	...
	def _model_test_(self, test_text):
		...
		score = self.model.evaluate(x=[input_ids,input_mask,input_pos], y=testy, verbose=1)
		print("the test loss for model: %s" %score[0])
		print("the test accuracy for model: %s" %score[1])
		return
		
	def _model_predict_(self, talk):
		input_ids, input_mask, input_type, input_pos = self._text_process([talk])
		self.model.load_weights(os.path.join(self.saved_models_dir, "bert_bilstm_crf_model.h5"))
		tag = self.model.predict([input_ids,input_mask,input_pos])[0][1:len(talk[0])+1]
		tag = self._id2tag_func(self._label_decoder(tag))
		return tag

	def _model_predict_batch(self, batch):
		input_ids, input_mask, input_type, input_pos = self._text_process(batch)
		self.model.load_weights(os.path.join(self.saved_models_dir, "bert_bilstm_crf_model.h5"))
		tags = self.model.predict([input_ids,input_mask,input_pos])
		tag = []
		for i in range(len(tags)):
			tag.append(tags[i][1:len(batch[i][0])+1])
		tag = [self._id2tag_func(self._label_decoder(t)) for t in tag]
		return tag

	def _text_process(self, text):
		Tokener = Tokenizer(self.vocab_dict)
		encoder = [Tokener.encode(first=doc[0], max_len=self.max_seq_len) for doc in text]
		input_ids = [i[0] for i in encoder]
		input_type = [i[1] for i in encoder]
		input_mask = [[0 if l==0 else 1 for l in i] for i in input_ids]
		input_pos = [[0]+[self._pos2id.get(t,0) for t in doc[1]]+[0] for doc in text]
		input_pos = pad_sequences(input_pos, self.max_seq_len, padding="post", truncating="post")
